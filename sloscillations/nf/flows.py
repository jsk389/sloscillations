...
class NSF_CL(nn.Module):
    ...
    def __init__(self, dim, K = 5, B = 3, hidden_dim = 8, base_network = FCNN):
        ...
        if dim % 2 == 0:
            ...
            self.f1 = base_network(dim // 2 + 1, (3 * K - 1) * (dim // 2 + 1), hidden_dim)
            self.f2 = base_network(dim // 2 + 1, (3 * K - 1) * (dim // 2 + 1), hidden_dim)
    ...
    def forward(self, x):
        ...
        if self.dim % 2 == 0:
            ...
            lower, upper = x[:, :self.dim // 2 + 1], x[:, (self.dim // 2 + 1):]
            out = self.f1(lower).reshape(-1, self.dim // 2 + 1, 3 * self.K - 1)            
        ...
        if self.dim % 2 == 0:
            ...
            out = self.f2(upper).reshape(-1, self.dim // 2 + 1, 3 * self.K - 1)            
        W, H, D = torch.split(out, self.K, dim = 2)
        W, H = torch.softmax(W, dim = 2), torch.softmax(H, dim = 2)
        W, H = 2 * self.B * W, 2 * self.B * H
        D = F.softplus(D)
    ...
    def inverse(self, z):
        ...
        if self.dim % 2 == 0:
            ...
            lower, upper = z[:, :self.dim // 2 + 1], z[:, (self.dim // 2 + 1):]
            out = self.f2(upper).reshape(-1, self.dim // 2 + 1, 3 * self.K - 1)            
        W, H, D = torch.split(out, self.K, dim = 2)
        W, H = torch.softmax(W, dim = 2), torch.softmax(H, dim = 2)
        W, H = 2 * self.B * W, 2 * self.B * H
